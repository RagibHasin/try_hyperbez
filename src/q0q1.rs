...
impl std::fmt::Display for AppData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, "{},{},{},{},{param_k},{param_c}", p1.x, p1.y, p2.x, p2.y)
    }
}
...
fn memoized_app_logic(data: &AppData) -> MemoizedState {
    ...
    let params = hb::solver::q0q1::solve(scale_down * cubicbez, data.param_k, data.param_c)
        .unwrap_or_else(|| {
    ...
    let frag_param_k = labeled_valued(
        "K: ",
        slider(data.param_k, 2., 3., 0.02).map_state(|data: &mut AppData| &mut data.param_k),
